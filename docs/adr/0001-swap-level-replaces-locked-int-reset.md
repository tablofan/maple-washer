...
- **Very late swaps remain expensive for high-`freshAPHP` classes, but cost is not monotonic.** Sweeping a Hero at 30k HP / 2k MP @ 180 gives: swap 40 → 603 resets, 80 → 696, 120 → 567, 160 → 811, 180 → 986. The optimizer trades pre-swap Main Stat allocation against post-swap Fresh HP Wash, so the best mix can sit in the middle; a late Swap Level no longer falsely forces an MP Wash on every preceding level.
