...
**Non-INT Pool**:
The STR/DEX/LUK AP that is still free to move into INT: each stat's value less its permanent floor (4, or the class's **First Job Requirement** minimum for the stat that has one), summed. An Assassin at 10 STR / 30 DEX / 10 LUK has a pool of 17 — 6 + 5 + 6, because Thief DEX is pinned at 25. The user still enters all four stats separately, but everything downstream works from the pool: which stat a movable point came from never changes the plan, and the player decides at execution time which to drain. The pool is what a positive **Shift to INT** spends, so it shrinks by the shift and then only grows — every AP the plan later puts into **Main Stat** above its floor is movable again, including the Base INT flushed there at the **Swap Level**. Magicians are the exception: their **Main Stat** IS INT, which is not in the pool, so theirs holds steady. Shown on the wizard's review step and as a level-table column. At target level the **Reset Base INT** step collapses any returned INT back into the **Main Stat**, so non-MainStat stat points consumed by the wash are not preserved (the player can manually redistribute via Cash Shop afterward if desired).
_Avoid_: side stats, secondary stats, Non-INT Stats Pool

**HP/MP Pool**:
A single shared counter of AP points the player has placed into HP or MP (via **AP Allocation** or `-stat +HP/MP` **AP Resets**). The game enforces: `-HP/MP +stat` resets require this pool to be non-empty (you can only reclaim AP you previously placed). `-stat +HP/MP` is unconstrained. MapleWasher assumes the user's **HP/MP Pool** is 0 at the start of the calculation — this is conservative (no historical wash credit). Before any planned Stale HP Wash, the schedule places at least one AP into HP or MP and keeps its paired reset pending until the stale washes finish. Usually this is fresh AP; Mage preparation without fresh AP explicitly pays for a stat reset to open the pool. A plan can skip **MP Wash** yet still use `-MP +MainStat` because one fresh AP can first seed HP and then be reclaimed after the stale wash.
...
**MP-Cap HP Wash**:
The Magician HP-wash endgame. Each level, fresh AP goes into MP and returns through `-MP +INT`. Keep the last paired reset pending while doing that level's `-MP +HP` stale washes, then restore the last INT. The MP left at the end of the level reserves room for the next level-up and every fresh MP allocation, including the temporary balance before its paired reset. All intermediate HP/MP values stay at or below 30,000; MP lost to a cap is never converted into HP. A 30,000 goal first reaches the cap at **Target Level**, not earlier (an already-capped input is historical state). For an MP goal above 29,970, the target level's fresh AP goes directly into INT: the final level-up fills MP, and no subsequent MP reset drains it. If a progressed character needs headroom before any fresh AP is available, the plan explicitly counts `-INT +HP` to open the HP/MP Pool and `-MP +INT` to close it after the preparation washes. Source for the underlying washing rates: Krythan's mage sheet + Shivering's "Comprehensive Guide to HP/MP Washing on Mages".
...
- **Mage:** *(optional)* pre-game **Shift to INT** → satisfy the 20 INT **First Job Requirement** as part of building **Base INT** → **MP Wash** (`+MP`, then `-MP +INT`) → continue MP Washing while **MP-Cap HP Washing** (`-MP +HP`) with enough room for subsequent gains → fill a 30k MP goal at **Target Level** using its natural level-up, with fresh AP directly into INT. **Magicians skip the Base-INT reset** because INT is already their Main Stat.
...
1. **Summary card** — projected final HP and MP beside their respective goals, total **AP Resets**, **NX Cost** (= AP Resets × 3,100), and **Time to save NX** (= NX Cost ÷ 6,500 NX-per-day-per-account, rounded up to whole days). Longer savings periods also show approximate months or years. The estimate assumes one account and no existing NX. Below these figures, **Key levels & INT** shows `Target Base INT` (`Final Base INT` for Magicians), `MP Wash Start Level`, and either the non-Mage **Swap Level** and `HP at Swap Level`, or the Magician **Stale HP Wash** start. **Where the AP Resets go** expands to show each reset type, operation, count and NX cost. Figures appear immediately, without counting up. **Edit goals** returns to the Goals step.
2. **Phase Plan** — a numbered, level-banded allocation guide matching the **Search space** decisions. Multi-action phases separate the engine's instructions into ordered actions while preserving all wording and pool constraints. A collapsible washing-term guide explains fresh AP, AP Reset notation, Fresh/Stale HP Wash and the HP/MP Pool.
3. **Level-by-level table** — 9 columns: Level, HP, MP, Base INT, **Non-INT Pool**, Fresh AP allocations, AP Reset transfers, AP Resets this level, Cumulative AP Resets. Allocations show counts and destinations (for example, `5 AP → MP`); transfers show counts, sources and destinations (for example, `2 × MP → INT` and `3 × MP → LUK`). These are recorded from the actual per-level calculations, including first-job requirements, split allocations, and deferred wash resets. They are totals, not an execution order; the **Phase Plan** remains the ordered guide for preserving the HP/MP Pool. Any upfront **Shift to INT** appears on the initial row as `Non-INT → INT` and is included in both that row's reset count and the cumulative total. The pool sits beside **Base INT** so the swap is visible: at **Swap Level** **Base INT** collapses to its starting value and the pool jumps by the same amount, because the flushed INT lands in **Main Stat** and is movable again. The pool is shown rather than **Main Stat** because it stays honest after a pre-game shift, which can come from any eligible non-INT stat. Collapsed by default; highlighted rows mark changes in AP actions, and the Target Level row is green.

When the user's inputs make the goal **infeasible**, the calculator shows an **infeasibility warning** in place of the Summary, naming the violated constraint. The two commonest are a goal beyond what the level allows — *“The most HP reachable at 1600 MP by level 50 is about 1,924 — short of the 30,000 HP goal.”* — and a goal below the class floor — *“MP Goal (1,000) is below the minimum possible MP (1,549) at level 50 for a Magician.”* Note that the two are easy to confuse: a Mage asking for 30k HP at level 50 trips the **MP floor** first, not the HP ceiling, because level 50 forces more MP than the request allows.
...
- The **MP Wash Start Level** milestone is labelled "Start MP Wash" with a value such as "Level 68" or "Not needed". **Swap Level** keeps its full name since it names an event, not a range.
- Magicians see "Final Base INT" because their INT is a projection at Target Level, not a chosen plateau. Their plan has no Swap Level or Base INT reset.

The Mage phase vocabulary also includes **Prepare MP headroom** (washing before the next level-up) and **Reach MP goal** (the final natural fill with fresh AP directly into INT). The calculator uses average natural and fresh-AP gains; final gains may saturate at the game cap, and any discarded MP is excluded from washing yields.
