...
_Known modelling artefact_: the engine levels first and washes the excess down afterwards, where a player washes down first and then levels. The MP generated and the washes performed are the same either way, so cost and final HP/MP are right — but when the **MP Goal** is itself 30,000 the level table's transient MP peak reads up to ~700 over the cap for the levels in this phase. Level-end MP is always capped. `verifyWithWalk` exempts cap-wash plans from the transient-peak check for this reason.
